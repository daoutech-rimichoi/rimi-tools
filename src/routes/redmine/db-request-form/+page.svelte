<script>
	...
	let details = $state('');
	let type = $state('');
	let environment = $state('custom'); // 검수, 운영, 직접입력
	let selectedSystems = $state([]);
	let customSystem = $state('');
	let table = $state('');
	let impact = $state('');
	let backup = $state('');
	let deploymentDateTime = $state(getDefaultDateTime());
	...
	// 선택한 환경에서 고를 수 있는 시스템 목록
	const availableSystems = $derived(systemOptions[environment] ?? []);

	// 환경이 바뀌면, 더 이상 선택할 수 없는 시스템은 선택 해제한다
	$effect(() => {
		if (environment === 'custom') return;
		const kept = selectedSystems.filter((s) => availableSystems.includes(s));
		if (kept.length !== selectedSystems.length) selectedSystems = kept;
	});

	// 출력에 쓰일 시스템 문자열
	const system = $derived(environment === 'custom' ? customSystem : selectedSystems.join(', '));
	...
	const output = $derived.by(() => {
		...
		const body = rows
			.map(({ label, value, trHeight, trHeightPt, isFirst, multiline }) => {
...
<ToolPage>
	<div class="grid grid-cols-1 gap-8 md:grid-cols-2">
		...
		<!-- Output Section -->
		<ResultPanel body={output} rows={26} />
	</div>
</ToolPage>
